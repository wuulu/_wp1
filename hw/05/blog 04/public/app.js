...
async function openProfile(userId) {
  ...
  profileMeta.textContent = `${user.post_count} posts · Joined ${new Date(user.created_at.replace(' ', 'T')).toLocaleDateString()}`;

  const postsRes = await fetch(`${API}?user_id=${userId}`);
  ...
  postsList.innerHTML = posts.map(post => `
    <div class="post-card">
      <div class="post-header">
        ...
        <div>
          <div class="post-username" onclick="openProfile(${post.user_id})">${escapeHtml(post.username || 'anonymous')}</div>
...
profileBack.addEventListener('click', () => switchTab(activeTab));
